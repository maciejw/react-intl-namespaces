...
let FormattedMessage$1 = class FormattedMessage$$1 extends FormattedMessage {
    ...
    render() {
        ...
        if (this.context.intlNamespace.includeMetadata) {
            ...
            const metadata = InltNamespaces.getMessageMetadata(this.props, currentNamespace);
    ...
    getChildContext() {
        ...
        const formatMessage = (messageDescriptor, values) => {
            ...
            if (this.context.intlNamespace.showIds) {
                return InltNamespaces.getResourceKey(messageDescriptor, getNameOfCurrentNamespace(), Object.getOwnPropertyNames(values));
            }
            else {
...
let IntlNamespaceProvider = class IntlNamespaceProvider extends Component {
    ...
    getChildContext() {
        ...
        return {
            intlNamespace: {
                ...
                loggingEnabled,
                missingMessage: messageDescriptor => {
                    const missingMessage = InltNamespaces.getMessageMetadata(messageDescriptor, namespace);
    ...
    namespaceLoadedNotification({ namespace: messagesNamespace, resource: messages, }) {
        ...
        if (includeNamespace.includes(messagesNamespace)) {
            messages = InltNamespaces.addNamespaceToResource(messages, messagesNamespace);
            const newState = {
                messages: Object.assign({}, this.state.messages, messages),
            };
            this.setState(newState);
...
export { FormattedMessage$1 as FormattedMessage, IntlProvider$1 as IntlProvider, IntlBackendProvider, IntlNamespaceProvider, InltNamespaces, defineMessages, ResourceProvider };
