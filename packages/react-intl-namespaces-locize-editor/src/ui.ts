const baseBtn =
  ...
  'border: none; font-weight: 300; height: 30px; line-height: 30px; padding: 0; ' +
  'text-align: center; min-width: 90px; text-decoration: none; ' +
...
export function initUI(on: () => void, off: () => void): (on: boolean) => void {
  ...
  cont.setAttribute(
    'style',
    'z-index: 2001; font-family: "Helvetica", "Arial", sans-serif; ' +
      'position: fixed; bottom: 20px; right: 20px; padding: 10px; ' +
      'background-color: #fff; border: solid 1px #1976d2; box-shadow: 0px 1px 2px 0px rgba(0,0,0,0.5);',
  );
  ...
  title.setAttribute(
    'style',
    'font-family: "Helvetica", "Arial", sans-serif; font-size: 14px; margin: 0 0 5px 0; color: #1976d2; ' +
      'font-weight: 300;',
  );
  ...
  turnOff.setAttribute(
    'style',
    `${baseBtn} display: none; background-color: #54A229;`,
  );
  turnOff.onclick = off;
  ...
  turnOn.setAttribute(
    'style',
    `${baseBtn} display: none; background-color: #D50000;`,
  );
  turnOn.onclick = on;
  ...
  const toggle = (state: boolean) => {
    turnOff.style.display = state ? 'block' : 'none';
    turnOn.style.display = !state ? 'block' : 'none';
  };
